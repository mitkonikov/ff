...
class FFRecurrentLayer(IFFRecurrentLayer):
    ...
    def forward(
        ...
        # Main equation
        return (
            self.beta * self.activation_fn(f + b + self.fb.unsqueeze(0)) + (1 - self.beta) * x_recc
        )
    ...
    def run_train(
        ...
        g_pos, _ = self.goodness(h_pos[index - 1], h_pos[index], h_pos[index + 1])
        g_neg, _ = self.goodness(h_neg[index - 1], h_neg[index], h_neg[index + 1])
